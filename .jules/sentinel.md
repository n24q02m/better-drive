...
## 2026-07-30 - Fix Systemd Unit File Injection
**Vulnerability:** Systemd unit files were generated by passing `os.Executable()` into `fmt.Sprintf` directly (`ExecStart=%s run`). If the application's path contained spaces, systemd would split the path incorrectly. If the path contained `%`, systemd would attempt to expand it as a specifier, causing unexpected behavior or failure.
**Learning:** When templating Systemd units from user-controlled or system-derived paths, you must guard against both command splitting and systemd's built-in `%` specifier expansion.
**Prevention:** Always escape `%` as `%%` (e.g., `strings.ReplaceAll(s, "%", "%%")`) before using it in a systemd unit template, and use Go's `%q` (which adds double quotes and escapes) to prevent whitespace splitting and quote injection.
