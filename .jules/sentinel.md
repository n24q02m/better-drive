...
## 2026-08-17 - Prevent Arbitrary Execution via Windows Explorer
**Vulnerability:** Passing unvalidated paths to `explorer` via `exec.Command` can lead to arbitrary file execution if the path points to an executable rather than a directory. `filepath.Clean` alone only provides lexical path cleaning, offering no real security benefit against malicious file execution.
**Learning:** When passing paths to external utilities like Windows `explorer`, you must validate the resolved absolute path or use `os.Stat` to confirm the target is strictly a safe directory before execution.
**Prevention:** Always use `os.Stat(path)` and verify `info.IsDir()` is true before passing user-controlled or configured paths to `explorer` to ensure only directories are opened.
