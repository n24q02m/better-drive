...
## 2026-08-21 - Verify directory target for Windows explorer execution
**Vulnerability:** When executing `explorer.exe` to open a local sync folder from the system tray on Windows, the application was using `filepath.Clean()` as the only validation. If `path` was pointing to a file or a malicious executable (instead of a directory), `explorer.exe` could execute it or behave unexpectedly, leading to a path traversal / local code execution risk.
**Learning:** `filepath.Clean()` only performs lexical cleaning. It does not verify the path's existence or its type. Passing unsanitized paths to commands like `explorer` is risky. We need to actively verify the target is a directory on disk.
**Prevention:** Always use `os.Stat(path)` and verify `info.IsDir()` is true before passing external paths to subprocesses like `explorer`, ensuring the command is only executed on safe directories.
