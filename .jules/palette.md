...
## 2024-08-15 - Actionable Tooltips for Disabled Status Elements
**Learning:** When using a disabled menu item as a passive status indicator (like `mStatus`), displaying an error state (e.g. `needs-resync`) without actionable advice leads to a dead-end user experience.
**Action:** Always provide the exact resolution steps or CLI commands (e.g. `better-drive sync --resync`) in the tooltip of disabled status elements to guide users out of broken states.
