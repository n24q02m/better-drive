...
## 2024-05-15 - Append actionable instructions to disabled tray menu items
**Learning:** Standard macOS and Windows system tray menus do not display tooltips on individual menu dropdown items, leaving users blind to why an item is disabled or how to recover from an error state.
**Action:** Append the disabled reason or recovery instruction directly to the system tray menu item's title when disabling it.
