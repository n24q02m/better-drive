...
## 2024-09-05 - Disabled Menu Item Explanation Visibility
**Learning:** Standard macOS and Windows system tray menus do not display tooltips on individual menu dropdown items. Relying solely on `MenuItem.SetTooltip` to explain why an item is disabled means users never see the explanation, leading to confusion.
**Action:** When disabling a contextual tray menu item, always append the disabled reason to the menu item's title in addition to setting the tooltip.
