...
## 2024-10-26 - Actionable Tooltips for Error States
**Learning:** When disabling a UI element due to a broken or error state (like needing a resync), passively stating the error leaves users stuck. Providing the exact command or action needed reduces frustration and improves usability.
**Action:** Always provide actionable instructions in the tooltip for disabled error states, such as the exact CLI command needed to resolve the issue.
