...
## 2025-02-13 - Actionable Error Tooltips
**Learning:** When a background application enters a broken state (like needing a resync), passively stating the error in the tray icon or status tooltips leaves the user stranded. Tooltips on primary status indicators are often the first place users look for help.
**Action:** Always provide actionable instructions (like specific CLI commands) in status and tray tooltips during error states, rather than just describing the error.
