...
## 2025-02-28 - Zero-allocation prefix match with EqualFold
**Learning:** Using `strings.HasPrefix(strings.ToUpper(str), "PREFIX")` in hot paths (like parsing OS environment variables loop) causes a hidden heap allocation because `strings.ToUpper` creates a new string.
**Action:** Replace it with `len(str) >= 4 && strings.EqualFold(str[:4], "GIT_")` to eliminate the heap allocation while keeping the case-insensitive logic readable.
