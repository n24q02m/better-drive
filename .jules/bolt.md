...
## 2024-08-20 - Replace strings.Split with iterative strings.Cut
**Learning:** When parsing large string outputs or multi-line configuration/shim files in Go, using `strings.Split` allocates a new slice and populates it with all split strings, causing unnecessary garbage collection overhead.
**Action:** Prefer using `strings.Cut` iteratively in a loop (e.g., `line, rest, _ := strings.Cut(dataStr, "\n")`) to eliminate slice allocations and GC overhead when parsing lines.
