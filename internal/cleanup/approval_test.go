...
func TestActivateApprovalBindsDraftAndRejectsExpired(t *testing.T) {
	...
	intent, err := ActivateApproval(approval, signature, publicKey, time.Unix(150, 0).UTC())
	if err != nil {
	...
	if _, err := ActivateApproval(approval, signature, publicKey, time.Unix(250, 0).UTC()); err == nil || !strings.Contains(err.Error(), "expired") {
