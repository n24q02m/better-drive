...
func cleanRelativePath(value string) (string, error) {
	...
	for _, part := range parts {
		...
		// ⚡ Bolt: using strings.EqualFold avoids strings.ToUpper heap allocations
		p := strings.TrimSuffix(part, ".")
		if len(p) == 3 {
			if strings.EqualFold(p, "CON") || strings.EqualFold(p, "PRN") || strings.EqualFold(p, "AUX") || strings.EqualFold(p, "NUL") {
				return "", fmt.Errorf("device name %q is forbidden", part)
			}
		} else if len(p) == 4 {
			if strings.EqualFold(p[:3], "COM") || strings.EqualFold(p[:3], "LPT") {
				if p[3] >= '1' && p[3] <= '9' {
					return "", fmt.Errorf("device name %q is forbidden", part)
				}
			}
