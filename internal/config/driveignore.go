...
func TranslateIgnoreLines(lines []string) []string {
	...
	// Pre-allocate slice capacity to prevent hidden re-allocations during append.
	out := make([]string, 0, len(lines))
	for _, raw := range lines {
		line := strings.TrimSpace(raw)
		// Use zero-allocation byte indexing instead of strings.HasPrefix for single-character checks.
		if line == "" || line[0] == '#' {
			continue
		}
		sign := "- "
