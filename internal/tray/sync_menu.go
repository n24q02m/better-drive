...
func syncMenuState(aggregate AggregateState) (enabled bool, title, tooltip string) {
	...
	case aggregate.State == syncloop.StateSyncing:
		return false, "Sync now (syncing)", "Sync is currently in progress"
	case aggregate.NeedsResync:
		return false, "Sync now (run sync --resync)", "Run better-drive sync --resync to rebuild the bisync baseline"
	case aggregate.State == syncloop.StatePaused:
		return false, "Sync now (paused)", "Cannot sync while paused"
	default:
...
func pauseMenuState(aggregate AggregateState) (enabled bool, title, tooltip string) {
	if aggregate.NeedsResync {
		return false, "Pause (run sync --resync)", "Run better-drive sync --resync before pausing"
	}
