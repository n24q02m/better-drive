...
func TestSyncMenuStatePausedDisablesWithReason(t *testing.T) {
	...
	if title != "Sync now (paused)" {
		t.Fatalf("paused sync title = %q, want %q", title, "Sync now (paused)")
	}
...
func TestSyncMenuMixedErrorAndNeedsResyncDisables(t *testing.T) {
	...
	if title != "Sync now (run sync --resync)" {
		t.Fatalf("needs-resync title = %q, want %q", title, "Sync now (run sync --resync)")
	}
...
func TestSyncMenuRegularErrorRemainsEnabledForRetry(t *testing.T) {
	...
	if title != "Sync now" {
		t.Fatalf("ordinary error title = %q, want %q", title, "Sync now")
	}
...
func TestPauseMenuNeedsResyncCannotMaskRecoveryState(t *testing.T) {
	...
	if title != "Pause (run sync --resync)" || !strings.Contains(tooltip, "better-drive sync --resync") {
